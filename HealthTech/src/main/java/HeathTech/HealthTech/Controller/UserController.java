...
import HeathTech.HealthTech.Entirty.Userlogin;
...
public class UserController {
    ...
    public ResponseEntity<?> cretaeuser(@RequestBody User user) {
        boolean exits = userService.existsByUsername(user.getUsername());

        boolean check = pattern.matcher(user.getEmail()).matches();
        ...
        if (userdatabase.existsByEmail(user.getEmail())) {
    ...
    public ResponseEntity<?> loginuser(@RequestBody Userlogin user) {
        ...
        try {
            ...
            User userdb=userService.finduser(user.getUsername());
    ...
    @PostMapping("/forget-passworduser")
    public ResponseEntity<?> forgetpassword(@RequestParam String email){
        emailService.sendMail(email,"Change Password",frontendUrl+"/user/reset-password");
    ...
    public ResponseEntity<?> forgatpassword(@RequestParam String email,@RequestParam String password) {
        try {
            User olduser = userdatabase.findByEmail(email);
            if (olduser == null) {
